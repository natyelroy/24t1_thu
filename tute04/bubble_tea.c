// bubble_tea.c
//
// Written by Natalie Leroy zID 7/03/2024
//
...
void print_final_order(struct bubble_tea order,
                        double total_cost);
                            // Update the stock after fulfilling the order
int update_stock(struct bubble_tea order, struct inventory inventory);
...
int main(void) {
    ...
    // Update the stock after fulfilling the order
    inventory.tea = update_stock(order, inventory);
...
// printing details of final order
...
// updating stock of tea in inventory
int update_stock(struct bubble_tea order, struct inventory inventory) {
    if (order.size == LARGE) {
        inventory.tea -= 2;
    } else {
